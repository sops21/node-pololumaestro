...
PololuMaestro.prototype.set8BitTarget = function (channel, target, onComplete) {
  ...
  this._send8BitCommand(0xFF, channel, target * 4, onComplete)
}
/**
 * Sets the servo target via the Pololu protocol
 */
PololuMaestro.prototype.setPololuTarget = function (deviceNumber, channel, target, onComplete) {
	// if(target < 0 || target > 254) {
	// 	LOG.error("PololuMaestro", "8 bit target value should be 0-254, was:", target);
	...
	// }
  console.log('Test')
	// this._serialPort.write([ 0xAA, 13, 0x04, 0].concat(ByteUtils.toLowAndHighBits(value)), onComplete);
  LOG.debug('PololuMaestro', 'Setting channel', channel, '8 bit target to', target, 'microseconds')
	/**
	 * Command
	 * Device Number
	 * Command with significant bit cleared
	 * target * 4?
	 *
	 */
  console.log(target)
  var commandData = [channel].concat(ByteUtils.toLowAndHighBits(target * 4))
	this._send8BitCommandPololuProtocol(deviceNumber, 0x04, commandData, onComplete)
}
...
PololuMaestro.prototype.digitalRead = function (channel, onValue) {
  ...
  this._serialPort.writeAndRead(
		[0x90, channel], function (data) {
  onValue(ByteUtils.fromLowAndHigh8Bits(data) >= 6000)
}
	)
}
...
PololuMaestro.prototype._send8BitCommandPololuProtocol = function (deviceNumber, command, commandData, onComplete) {
	// this._send8BitCommandPololuProtocol(deviceNumber, 0x04, commandData, onComplete);
  console.log('Device Number ' + deviceNumber + ' Command' + command + ' Command Data' + commandData)
  this._serialPort.write([ 0xAA, deviceNumber, command].concat(commandData), onComplete)
}
